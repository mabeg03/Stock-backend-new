...
class HybridStockPredictor:
    ...
    def predict(self, ticker: str, days: int = 1):
        ...
        # 🌟 Model predicts % change, not raw price.
        predicted_return = float(self.model.predict(X_scaled)[0])

        last_price = float(closes[-1])
        predicted_price = last_price * (1 + predicted_return)

        # Optional sanity check — limit crazy predictions
        predicted_price = max(predicted_price, last_price * 0.9)
        predicted_price = min(predicted_price, last_price * 1.1)
